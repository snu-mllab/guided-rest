...
@register_adv_est(AdvantageEstimator.HGAE)  # or simply: @register_adv_est("hgae")
def compute_hgae_advantage_return(
    token_level_rewards: torch.Tensor,
    values: torch.Tensor,
    response_mask: torch.Tensor,
    gamma: torch.Tensor,
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(gen_len)):
            cumrewards += token_level_rewards[:, t]
            delta = cumrewards + gamma * nextvalues - values[:, t]
            lastgaelam_ = delta + gamma * lam * lastgaelam

            # skip values and TD-error on observation tokens
            cumrewards *= 1 - response_mask[:, t]
            nextvalues = values[:, t] * response_mask[:, t] + (1 - response_mask[:, t]) * nextvalues
            lastgaelam = lastgaelam_ * response_mask[:, t] + (1 - response_mask[:, t]) * lastgaelam
...
@register_policy_loss("hppo")
def compute_policy_loss_hppo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    # Clamp negative_approx_kl for stability
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    gen_len = negative_approx_kl.shape[-1]
    for t in reversed(range(gen_len - 1)):
        negative_approx_kl[:, t] += (1 - response_mask[:, t + 1]) * negative_approx_kl[:, t + 1]
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = verl_F.masked_mean(-negative_approx_kl, response_mask)
