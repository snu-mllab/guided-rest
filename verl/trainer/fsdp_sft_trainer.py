...
class FSDPSFTTrainer:
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True, n_micro_batches=1):
        ...
        loss_mask = batch["loss_mask"].to(self.device_name)
        labels = torch.where(loss_mask == 0, -100, input_ids)
            ...
            if not use_sp:
                ...
                if self.config.use_remove_padding:
                    ...
                    labels_rmpad = index_first_axis(
                        rearrange(labels.unsqueeze(-1), "b s ... -> (b s) ..."), indices
                    ).transpose(0, 1)
                ...
                raise NotImplementedError("Sequence parallel is not implemented yet.")
